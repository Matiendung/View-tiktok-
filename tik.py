...
class Main:
    def __init__(self):
        ...
        cursor.hide()
        txt=self.format(" Ma Tiến Dũng ", " ID Video Tik Tok :  ")
        YourId=input(txt)
        self.videos = YourId
...
╚═════╝░YTB : Dũng Brayl ░╚═╝░░╚══╝                   \n                    #Ma Tiến Dũng\n\n\n\n\n"""
    ...
    def title(self):
        ...
        while True:
            ...
            try:
                ...
                os.system(
                    f"title Ma Tiến Dũng © 2022  x  Zviews ^| Views: {views} ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                time.sleep(0.5)
                ...
                os.system(
                    f"title Ma Tiến Dũng © 2022  x  Zviews ^| Views: ERROR ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                pass
    def solve_captcha(self, sessid):
        try:
            ...
            print(self.format(" Ma Tiến Dũng ",f" Solved captcha ! | {captcha_answer}"))
            return alpha_key
        ...
        while True:
            try:
                ...
                if "This service is currently not working" in decryped_answer:
                    ...
                    print(self.format(" Ma Tiến Dũng ",f" Delay: {timer}     "), end="\r")
                    start = time.time()
                    while time.time() < start + timer:
                        print("\r", end="")
                        print(self.format(" Ma Tiến Dũng ", f" Delay: {round((start + timer) - time.time())}       "),end="\r")
                        time.sleep(1)
                    continue
                ...
                if latency > 3:
                    print(self.format(" Ma Tiến Dũng ",f" Sent views ID: {aweme_id}"))
...
class Menu:
    def format(self, symbol, text):
        ...
        return f"""   {Col.Symbol(symbol, self.lblue, self.blue)} {self.lblue}{text}{Col.reset}"""
    def main(self):
        banner=requests.get("https://raw.githubusercontent.com/lequangdung2007/profile/main/jungx").text
